...
<body class="bg-gray-50">
        ...
        <div class="container mx-auto px-4">
            <div class="flex justify-between items-center">
                <div class="flex items-center space-x-4">
                    <span><i class="fas fa-envelope mr-2"></i>hotronify@gmail.com</span>
                    <span><i class="fas fa-phone mr-2"></i>0898 686 001</span>
                </div>
                <div class="flex items-center space-x-4">
                ...
                <div class="flex items-center space-x-8">
                    ...
                    <div class="hidden lg:flex items-center space-x-6">
                        ...
                        <div class="relative group">
                            <button onclick="toggleDropdown('servicesDropdown')" class="text-gray-700 hover:text-teal-600 font-medium smooth-transition flex items-center">
                                Dịch Vụ
                                <i class="fas fa-chevron-down ml-1 text-xs"></i>
                            </button>
                            <div id="servicesDropdown" class="absolute hidden bg-white mt-2 py-3 w-56 rounded-lg shadow-xl z-50 border border-gray-100">
                                <a href="/packages.php" class="block px-4 py-2 hover:bg-teal-50 text-gray-700 hover:text-teal-600 smooth-transition">
                                    <i class="fas fa-server mr-2"></i>VPS NAT IP
                                </a>
                                <a href="/packages.php" class="block px-4 py-2 hover:bg-teal-50 text-gray-700 hover:text-teal-600 smooth-transition">
                                    <i class="fas fa-rocket mr-2"></i>VPS Cheap
                                </a>
    ...
    <!-- Contact Widget -->
    <div id="contactWidget" class="fixed bottom-8 right-8 z-40">
        <button id="contactToggle" class="w-14 h-14 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white rounded-full shadow-lg flex items-center justify-center transition-all hover:scale-110">
            <i class="fas fa-comments text-xl"></i>
        </button>
        
        <div id="contactDropdown" class="hidden absolute bottom-16 right-0 bg-white rounded-lg shadow-xl p-4 w-64">
    ...
    <div id="notificationsContainer" class="fixed top-20 right-4 z-50 space-y-2 max-w-sm">
    ...
    <script>
        ...
        function toggleContactWidget() {
            ...
            // Load contact settings if not loaded
            if (!dropdown.classList.contains('hidden') && !dropdown.dataset.loaded) {
                loadContactSettings();
                dropdown.dataset.loaded = 'true';
            }
            
            // Close when clicking outside
            document.addEventListener('click', function closeContact(e) {
                if (!e.target.closest('#contactWidget')) {
                    dropdown.classList.add('hidden');
        ...
        function loadContactSettings() {
            fetch('api/contact.php')
                ...
                .then(data => {
                    ...
                    if (data.length === 0) {
                        contactList.innerHTML = '<p class="text-gray-500 text-sm">No contact information available</p>';
                        return;
                    }
                    
                    data.forEach(contact => {
                        const item = document.createElement('div');
                        item.className = 'flex items-center space-x-2 p-2 hover:bg-gray-50 rounded';
                        
                        let linkHtml = '';
                        if (contact.contact_type === 'email') {
                            linkHtml = `<a href="mailto:${contact.contact_value}" class="flex items-center space-x-2 text-gray-700 hover:text-blue-600">`;
                        } else if (contact.contact_type === 'phone') {
                            linkHtml = `<a href="tel:${contact.contact_value}" class="flex items-center space-x-2 text-gray-700 hover:text-blue-600">`;
                        } else {
                            linkHtml = `<a href="${contact.contact_value}" target="_blank" class="flex items-center space-x-2 text-gray-700 hover:text-blue-600">`;
                        }
                        
                        item.innerHTML = `
                            ${linkHtml}
                                <i class="${contact.icon} text-blue-600"></i>
                                <div>
                                    <div class="text-sm font-medium">${contact.display_name}</div>
                                    <div class="text-xs text-gray-500">${contact.contact_value}</div>
                                </div>
                            </a>
                        `;
        ...
        function loadNotifications() {
            const currentPage = window.location.pathname.replace(/^\/|\.php$/g, '') || 'home';
            fetch(`api/notifications.php?page=${currentPage}`)
                .then(response => response.json())
                .then(data => {
        ...
        function createNotificationElement(notification) {
            ...
            div.className = `${colorClass} text-white px-4 py-3 rounded-lg shadow-lg transform transition-all duration-300 translate-x-full`;
            div.innerHTML = `
                <div class="flex items-start">
                    <i class="fas fa-${getNotificationIcon(notification.type)} mr-3 mt-1"></i>
                    <div class="flex-1">
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Add event listeners
            document.getElementById('contactToggle').addEventListener('click', toggleContactWidget);
